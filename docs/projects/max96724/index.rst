...
The :adi:`MAX96724 DPHY Evaluation Kit (EV Kit) <MAX96724F-BAK-EVK>` provides a proven design and reliable platform
to evaluate the :adi:`MAX96724` device using standard FAKRA coaxial cables.
...
The data path (through the deserializer) and the writing in memory are illustrated in the block design.

MAX96724 block design
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

.. image:: max96724_hdl.svg
   :width: 800
   :align: center
   :alt: MAX96724 EVKIT HDL-related block design
...
-  The user guide that exemplify the system interfaced using :adi:`AD-GMSLCAMRPI-ADP adaptor <AD-GMSLCAMRPI-ADP>`
   is available in wiki `page <https://wiki.analog.com/resources/eval/user-guides/ad-gmslcamrpi-adp/ug_amd_kria>`_
-  `GMSL drivers/dts <https://github.com/analogdevicesinc/linux/tree/gmsl/xilinx_v6.1_LTS>`_
